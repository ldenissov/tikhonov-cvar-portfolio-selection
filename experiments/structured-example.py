...
from model.operators import (
    build_problem, lipschitz_bound, objective_nu, phi_sparse, phi_turnover,
    pseudogradient
)
...
# Number of distinct assets behind the K = 3 columns, the first two assets are
# identical and the third one is a separate asset
K_distinct = 2
...
# Return distribution of the two distinct assets, the full expected return
# vectors are mu_1 = (0.05, 0.05, 0.03) and mu_2 = (0.02, 0.02, 0.07)
sigma = 0.20
rho = 0.3
mu_distinct = np.array([[0.05, 0.03],     # account 1
                        [0.02, 0.07]])    # account 2
...
# Market impact, only Lambda_12 = Lambda_21 = 0.4 differs from the standard
# example, so that trading either of the two identical listings has the same
# price effect
Lambda = np.array([[0.4, 0.4, 0.2],
                   [0.4, 0.4, 0.2],
                   [0.2, 0.2, 0.4]])
...
# Returns of the two distinct assets, drawn once and kept fixed for both runs.
# The first column is copied, so r_nu(omega) = (r~_nu,1, r~_nu,1, r~_nu,2)
q = draw_normal(N, K_distinct, S, mu_distinct,
                simple_cov(K_distinct, sigma, rho), seed=seed_returns)
r = q[:, :, [0, 0, 1]]
...
plt.title("Lower-level residual (structured example)")
plt.grid(True, which="both", alpha=0.3)
plt.legend()
plt.tight_layout()

plt.savefig(figure_dir / "structured_residual.pdf", bbox_inches="tight")
plt.savefig(figure_dir / "structured_residual.png", dpi=300, bbox_inches="tight")
plt.close()
...
write_latex_table(
    table_dir / "table6_structured_portfolios.tex",
    ["", r"$y_{\nu,1}$", r"$y_{\nu,2}$", r"$y_{\nu,3}$"],
    rows_portfolios,
    "Computed portfolios in the structured example.",
    "tab:structured-portfolios",
    placement="H",
)
...
write_latex_table(
    table_dir / "table7_structured_comparison.tex",
    ["Quantity", "Value"],
    ...
    "Distances and lower-level residuals in the structured example.",
    "tab:structured-comparison",
    placement="H",
)


################################################################################
#  Table of the selection functions and Expected Objective Values
################################################################################

rows_cross = []

for selection in selections:
    z = solutions[selection]
    value_sp = phi_sparse(z, N, K, xi, eta)
    value_turn = phi_turnover(z, N, K, v=v)
    objectives = [objective_nu(z, nu, r, b, v, Lam, alpha_cvar, eps)
                  for nu in range(N)]
    rows_cross.append([f"at $z^*_{{\\mathrm{{{selection[:4]}}}}}$"]
                      + [f"{val:.6f}"
                         for val in [value_sp, value_turn] + objectives])

write_latex_table(
    table_dir / "table8_structured_cross.tex",
    ["", r"$\Phi_{\mathrm{sp}}$", r"$\Phi_{\mathrm{turn}}$",
     r"$\mathbb{E}[f_1]$", r"$\mathbb{E}[f_2]$"],
    rows_cross,
    "Selection functions and expected objective values in the structured example.",
    "tab:structured-cross",
)
...
# The first column is copied as above, so the two identical assets stay
# identical for every sample size.
q_pool = draw_normal(N, K_distinct, max(sample_sizes), mu_distinct,
                     simple_cov(K_distinct, sigma, rho), seed=seed_returns)
r_pool = q_pool[:, :, [0, 0, 1]]
...
write_latex_table(
    table_dir / "table_structured_sample_size.tex",
    ["$S$", "Residual",
     r"$\|z^*_S - z^*_{S_{\max}}\|$", r"$\max_\nu |u_\nu|$"],
    rows_samples,
    "Effect of the sample size in the structured example.",
    "tab:structured-sample-size",
)
